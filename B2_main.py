...
DEBUG_SECRET = os.environ.get("FLASK_SECRET_KEY", "fallback_secret")  

@app.route("/flushredis", methods=["POST"])
def flush_redis():
    """Flushes all Redis data — use only for debugging."""
    secret = request.form.get("secret")
    if secret != DEBUG_SECRET:
        return "Unauthorized", 403
    try:
        redis_client.flushall()
        return "Redis flushed successfully", 200
    except Exception as e:
        return f"Error flushing Redis: {e}", 500


@app.route("/redisstatus", methods=["GET"])
def redis_status():
    """Shows Redis state: pending queue size, cache size, etc."""
    secret = request.args.get("secret")
    if secret != DEBUG_SECRET:
        return "Unauthorized", 403
    try:
        pending_len = redis_client.llen("pending_attendance")
        cache_size = redis_client.hlen("attendance_cache")
...
def health_check():
    """Checks Redis and Google Sheet connectivity."""
    secret = request.args.get("secret")
    if secret != DEBUG_SECRET:
        return "Unauthorized", 403
    status = {}
